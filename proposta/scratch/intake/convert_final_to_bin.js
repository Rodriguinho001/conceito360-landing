...
console.log('--- FINAL GEOMETRY INTAKE: POSITION + NORMAL + UV ---');
...
loader.load("DATA_PLACEHOLDER", (gltf) => {
    ...
    // Se a geometria não estiver indexada, cria um índice falso ou exporta direto
    let numTriangles = 0;
    if (idx) {
        numTriangles = idx.count / 3;
    } else {
        numTriangles = pos.count / 3;
    }
    ...
    for (let i = 0; i < numTriangles * 3; i++) {
        const vertexIndex = idx ? idx.array[i] : i;
