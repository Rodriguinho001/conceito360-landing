...
(async () => {
    const glbBuffer = fs.readFileSync('scratch/morro_trimmed.glb');
...
window.renderView = function(viewType) {
    ...
    await page.screenshot({ path: 'scratch/glb_perspective.png' });
    
    await page.evaluate(() => window.renderView('top'));
    await page.screenshot({ path: 'scratch/glb_top.png' });
    
    await page.evaluate(() => window.renderView('side'));
    await page.screenshot({ path: 'scratch/glb_side.png' });
    
    await page.evaluate(() => window.renderView('wireframe'));
    await page.screenshot({ path: 'scratch/glb_wireframe.png' });
    
    console.log('GLB screenshots captured');
    await browser.close();
})();
